...
export async function HomeScreen(container, sets, navigate) {
    container.innerHTML = `
  ...
  `;
    const listEl = container.querySelector('#sets-list');
    if (sets.length === 0) {
        listEl.innerHTML = `
    ...
    const cards = await Promise.all(sets.map(meta => renderSetCard(meta, navigate)));
...
async function renderSetCard(meta, _navigate) {
