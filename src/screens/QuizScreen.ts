import type { Question, QuizMode, QuizSession, Screen } from '../types';
...
interface State {
  ...
  mode: QuizMode;
  answeredQuestion: Question | null; // フィードバック表示中の問題（currentIndexは既に進んでいる）
}

export async function QuizScreen(
  ...
  const state: State = { session, phase: 'question', selectedChoice: null, mode, answeredQuestion: null };
...
function renderChoices(q: Question, state: State): string {
  return q.choices
    .map(choice => {
      let cls = 'choice-btn';
      if (state.phase === 'feedback') {
        if (choice === q.answer) cls += ' choice-btn--correct';
        else if (choice === state.selectedChoice) cls += ' choice-btn--incorrect';
...
function renderFeedback(q: Question, state: State): string {
